...
function acontece(alunosArray, quantidade){
    alunos = []
    for(let i = 0; i < alunosArray.length; i++){
        if(alunosArray[i]<=0){
            alunos.push(alunosArray[i])           
        }
    }
    return alunos.length >= quantidade
}

function aberturas(arrays, quantidade){
    aulasAconteceram = []
    for(let i = 0; i < arrays.length; i++){
        aulasAconteceram.push(acontece(arrays[i], quantidade))
    }
